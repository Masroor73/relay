...
- **Graceful shutdown implemented for the processor first, `cmd/server` deliberately not bundled in** - discovered as a real gap in already-shipped Milestone 1 code while building the processor's shutdown handling (ENG-29). Tracked as its own issue (related to ENG-29 and ENG-5) rather than silently folded into ENG-29 or left as an unaddressed doc inconsistency - ARCHITECTURE.md §11 corrected to state this honestly rather than continuing to claim both services were covered.
